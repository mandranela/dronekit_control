...
def is_target_reached(self, target_location):
    '''
    This function takes in a target location and checks if the current location of a vehicle is within 0.1 meters of the target location.
    
    :param target_location: A location object representing the target location that the vehicle needs to reach.   
    
    :return bool: True if the vehicle has reached the target location, otherwise it returns False.
    '''
    CLOSE_ENOUGH = 0.1

    if abs(self.location.global_relative_frame.lat - target_location.lat) > CLOSE_ENOUGH / 111111 or \
    abs(self.location.global_relative_frame.lon - target_location.lon) > CLOSE_ENOUGH / 111111 or \
    abs(self.location.global_relative_frame.alt - target_location.alt) > CLOSE_ENOUGH:
        return False
    else:
        return True
...
def get_distance_metres(aLocation1:LocationGlobalRelative, aLocation2:LocationGlobalRelative):
    ...
    return math.sqrt(math.pow(math.sqrt((dlat*dlat) + (dlong*dlong)) * 1.113195e5, 2) + dlat*dalt)


def get_location_heading(original_location, distance, heading):
    """
